...
| 列（depth=20時） | 内容 |
|-----------------|------|
| AF列 | 呼び出しメソッドのJavadoc（緑色フォント） |
| AG列 | 呼び出しメソッド内にSQL文がある場合: `●` |
| AH列 | 呼び出しメソッド内のSQL文 |
| AI列 | 呼び出しメソッド内にHTTPリクエストがある場合: `●` |
| AJ列 | HTTPリクエスト詳細（例: `GET - https://api.example.com/users, POST - https://api.example.com/orders`） |
| AK列 | 呼び出しメソッドの検出ワード（hitWords） |

その他:

- フォント: Meiryo UI
- Javadoc列（動的列）: 緑色フォント
- C〜E列の幅: 30、Javadoc列の幅: 30
- L列以降の列幅: 5
- すべてのセルは上下中央揃え
