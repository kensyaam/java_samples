...
「モデルデータ定義」シートに出力されたモデル一覧の中から、特定のクラスと抽出したいJSONキーを指定して、必要な行だけをフィルター表示できるシートです。

**機能と使い方:**
- **対象クラス(完全修飾名)**: B2セルに対象の完全修飾クラス名を入力します。
- **抽出対象JSONキー**: B5〜B204セルに抽出したい「JSONキー（パス表記）」を縦に入力します（最大200個まで指定可能）。
- 入力に応じて、D5セル以降に該当するモデルデータ定義の行がExcel関数によって自動的に抽出・展開されます。
- 指定されたJSONキーの親キー（例：`user.address.zipCode` を指定した際の `user` や `user.address`）も自動的にフィルタ結果に含まれます。
- 末尾の「サンプルJSON」行は完全修飾クラス名が設定されていないため、フィルター結果から自動で除外されます。

> **注意**: このフィルターシートはExcelの動的配列数式（スピル）を使用しています。`FILTER` 関数をサポートする Excel 2021 以降 または Microsoft 365 環境で開く必要があります。入力セル以外の領域は保護されており、誤って数式を削除する心配はありません。
