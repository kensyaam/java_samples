...
Excelファイルに「モデルデータ定義」シートが作成され、以下のカラムが出力されます。
...
| E | 内部要素/参照 | 配列やオブジェクトの内部型（クリックで該当クラスへジャンプ） |
| F | 必須 | ● または 空白 |
| G | サンプル値 | 型に応じた自動生成値 |
| H | Javaフィールド名 | フィールド変数名 (Javaヘッダー色: 緑) |
| I | Java型 | Javaの型名 (Javaヘッダー色: 緑) |
| J | 定義元クラス | 親クラスから継承したフィールドの場合、親クラス名を表示 (Javaヘッダー色: 緑) |
